...
/**
 * Kotlin front door to libsloth.so — the same frontend-free IME state
 * machine the fcitx5/IBus engines use (engine/common/core.h), driven on-device
 * with an ONNX decoder instead of the slothd Unix socket.
 *
 * There is exactly one [Core] per InputMethodService instance; it owns a native
 * SlothSession behind [handle]. Call [init] once, then drive the IME loop.
 *
 * ## Threading
...
class Core {
    ...
    // ---- native surface (bound by Java_com_sloth_ime_Core_*) -----------
