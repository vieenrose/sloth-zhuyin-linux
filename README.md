...
<p align="center"><img src="docs/android-boox-demo-v11.gif" width="380" alt="Android 原生輸入法(BOOX e-ink 實錄,25M 三值模型):逐鍵打「晚上熬夜看world cup,白天在louisa」——整句正確免選字、中英自動切換(world cup／louisa 保留英文)、每鍵即時反白"></p>
...
<p align="center"><img src="docs/score_vs_latency.png" width="620" alt="品質 vs 裝置端延遲(BOOX SD662):25M 三值在約 9ms/6 音節達免選字 76%,與只有 4M 的 int8 同速卻大勝;又快又準,勝過又慢又差的 12M int8(13ms / 免選字 72)"></p>

從零訓練的 **SlothE-T 25M 三值(W1.58A8)雙向編碼器**:在 BOOX(SD662)實測延遲下是
**Pareto 最佳**——約 **9ms/6 音節**就有 **免選字 76%**,和只有 4M 的 int8 同速,卻同時
**又快又準**贏過 12M int8(13ms、免選字 72)。**在此規模三值勝 int8**(與 11.6M 相反);
架構、GGUF 與完整重現流程見 [Luigi/slothe-t-25m-zhuyin](https://huggingface.co/Luigi/slothe-t-25m-zhuyin)。
（\* 25M 三值延遲由實測形狀的 TQ2_0 / I²_S kernel 推估。）
