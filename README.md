# 懶 Slothing — LLM 注音輸入法

**打注音,模型整句轉換。** 一顆從零訓練的 3.8M 語言模型在本機把注音解碼成
...
| | |
|---|---|
| **免選字整句轉換** | 微軟新注音式即打即轉;230 句免選字基準 **74%**(裝置端與桌面同分) |
...
| **完全離線** | 4.9 MB int8 ONNX 模型本機執行,零雲端、零遙測 |
...
**雙向編碼器**(非自回歸,一次前向)而非因果 LM:3.8M 參數(Hyperband NAS 於
sub-5M 空間搜出)、g2pW 語境讀音標註訓練、**字提示通道**(與輸出層權重共享,
...
| 基準 | 分數 |
|---|---|
| 230 句免選字(整句全對)| **74%**(172/230;Android 裝置端與桌面逐句一致 99%)|
...
SlothLM-E 3.8M(NAS＋g2pW)· 字提示通道(選字重評分/文件語境/錯字修復)·
