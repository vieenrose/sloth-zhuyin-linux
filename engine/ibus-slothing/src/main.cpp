// Slothing IBus engine — the IBus adapter over the shared frontend-free IME
...
#define IBUS_TYPE_SLOTHING_ENGINE (ibus_slothing_engine_get_type())

typedef struct _IBusSlothingEngine IBusSlothingEngine;
typedef struct _IBusSlothingEngineClass IBusSlothingEngineClass;

struct SlothingImpl; // C++ state, owned by the GObject

struct _IBusSlothingEngine {
    IBusEngine parent;
    SlothingImpl *impl;
};

struct _IBusSlothingEngineClass {
    IBusEngineClass parent;
};

GType ibus_slothing_engine_get_type();
...
struct SlothingImpl {
    IBusSlothingEngine *obj = nullptr;
    ...
    void loadPhoneticTable() {
        ...
        if (const char *env = std::getenv("SLOTHING_PHONETIC_TABLE")) {
            ...
            dirs.push_back("/usr/local/share/slothing/phonetic_table.tsv");
            dirs.push_back("/usr/share/slothing/phonetic_table.tsv");
...
G_DEFINE_TYPE(IBusSlothingEngine, ibus_slothing_engine, IBUS_TYPE_ENGINE)

static gboolean ibus_slothing_engine_process_key_event(IBusEngine *engine,
                                                       guint keyval,
                                                       guint keycode,
                                                       guint modifiers) {
    (void)keycode;
    auto *self = reinterpret_cast<IBusSlothingEngine *>(engine);
    return self->impl->processKey(keyval, modifiers) ? TRUE : FALSE;
}

static void ibus_slothing_engine_focus_out(IBusEngine *engine) {
    auto *self = reinterpret_cast<IBusSlothingEngine *>(engine);
    self->impl->flushPending();
    IBUS_ENGINE_CLASS(ibus_slothing_engine_parent_class)->focus_out(engine);
}

static void ibus_slothing_engine_reset(IBusEngine *engine) {
    auto *self = reinterpret_cast<IBusSlothingEngine *>(engine);
    self->impl->flushPending();
    IBUS_ENGINE_CLASS(ibus_slothing_engine_parent_class)->reset(engine);
}

static void ibus_slothing_engine_disable(IBusEngine *engine) {
    auto *self = reinterpret_cast<IBusSlothingEngine *>(engine);
    self->impl->flushPending();
    IBUS_ENGINE_CLASS(ibus_slothing_engine_parent_class)->disable(engine);
}

static void ibus_slothing_engine_candidate_clicked(IBusEngine *engine,
...
static void ibus_slothing_engine_init(IBusSlothingEngine *self) {
...
static void ibus_slothing_engine_destroy(IBusObject *obj) {
    auto *self = reinterpret_cast<IBusSlothingEngine *>(obj);
    delete self->impl;
    self->impl = nullptr;
    IBUS_OBJECT_CLASS(ibus_slothing_engine_parent_class)->destroy(obj);
}

static void ibus_slothing_engine_class_init(IBusSlothingEngineClass *klass) {
    IBusEngineClass *engine_class = IBUS_ENGINE_CLASS(klass);
    engine_class->process_key_event = ibus_slothing_engine_process_key_event;
    engine_class->focus_out = ibus_slothing_engine_focus_out;
    engine_class->reset = ibus_slothing_engine_reset;
    engine_class->disable = ibus_slothing_engine_disable;
    engine_class->candidate_clicked = ibus_slothing_engine_candidate_clicked;
    IBUS_OBJECT_CLASS(klass)->destroy = ibus_slothing_engine_destroy;
}
...
int main(int argc, char **argv) {
    ...
    if (!ibus_bus_is_connected(bus)) {
        g_printerr("ibus-engine-slothing: cannot connect to the IBus daemon "
                   "(is ibus running?)\n");
        return 1;
    }
    ...
    ibus_factory_add_engine(factory, "slothing", IBUS_TYPE_SLOTHING_ENGINE);

    if (byIbus) {
        ibus_bus_request_name(bus, "org.freedesktop.IBus.Slothing", 0);
    } else {
        // standalone/dev: register the component directly
        IBusComponent *component = ibus_component_new(
            "org.freedesktop.IBus.Slothing", "Slothing (LLM zhuyin)", "0.1",
            "LGPL-2.1-or-later", "sloth-zhuyin-linux",
            "https://github.com/vieenrose/sloth-zhuyin-linux", "", "");
        ibus_component_add_engine(
            component,
            ibus_engine_desc_new("slothing", "Slothing 注音",
                                 "Slothing: LLM-powered zhuyin IME "
                                 "(libchewing-free)",
