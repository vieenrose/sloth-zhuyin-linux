#!/usr/bin/env bash
# Headless end-to-end test: private dbus session + ibus-daemon + the slothing
# engine + the smoke client. Needs a running slothingd (real decode) and the
# built binaries:
#   ENGINE=/path/to/ibus-engine-slothing SMOKE=/path/to/smoke ./run-smoke.sh
set -e
