// Headless end-to-end smoke test for ibus-engine-sloth: connects to a
// (private) ibus-daemon as a client, focuses an input context on the
// "sloth" engine, types dachen keys, and asserts what gets committed.
// Run via run-smoke.sh (dbus-run-session + ibus-daemon + engine + this).
// Requires a running slothd (real decode) — the same bar as the web
// demo's parity suite.
#include <cstdio>
#include <cstring>
#include <ibus.h>
...
int main() {
    ...
    ibus_input_context_set_engine(ctx, "sloth");
