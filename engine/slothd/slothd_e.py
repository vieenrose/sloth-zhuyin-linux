#!/usr/bin/env python3
"""slothd-e: SlothLM-E (ONNX encoder) decode daemon for the fcitx5 engine.

Drop-in replacement for the llama.cpp slothd, speaking the same socket
...
accepted but unused (the encoder is context-free by design).
  ...
  python3 engine/slothd/slothd_e.py \
...
def default_socket_path():
    if os.environ.get("SLOTHD_SOCKET"):
        return os.environ["SLOTHD_SOCKET"]
    if os.environ.get("XDG_RUNTIME_DIR"):
        return os.path.join(os.environ["XDG_RUNTIME_DIR"], "slothd.sock")
    return "/tmp/slothd.sock"


LEARN_PATH = os.path.expanduser("~/.local/share/sloth/learn.tsv")
...
class Decoder:
    def __init__(self, model_dir, table_path):
        ...
        if not os.path.exists(onnx):
            sys.exit(f"slothd-e: no model at {model_dir}/model_quantized.onnx"
...
def main():
    ...
    srv.listen(8)
    print(f"slothd-e listening on {args.socket} "
